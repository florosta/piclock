...
import VolumeSheet from './views/VolumeSheet'
...
export default function App() {
  ...
  const settings = useSettings(backlight.setFullBrightness)
  ...
  return (
    <>
      ...
      <ClockView
        ...
        onShowPicker={() => { episodes.refresh(); setShowPicker(true) }}
        onShowAlarms={() => setShowAlarms(true)}
        onShowHA={() => { house.refresh(); setShowHA(true) }}
        onShowVolume={() => setShowVolume(true)}
      ...
      {showVolume && (
        <VolumeSheet volume={volume} onClose={() => setShowVolume(false)} />
      )}
